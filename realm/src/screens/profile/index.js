...
import { realm, Tables } from '../../database';
...
class Profile extends React.Component {
  ...
  handleUpdateAction = async () => {
    const { firstname, lastname, email, isDatabase } = this.state;
    const { user } = this.props;
    if (!checkEmpty(firstname)) {
      ...
      if (!isDatabase) {
        ...
        try {
          realm.write(() => {
            const test = realm.create(Tables.USERS, {
              id: params.user.id,
              firstName: firstname,
              lastName: lastname,
              email: email
  ...
  render() {
    const { email, firstname, lastname, emailError, firstnameError, lastnameError } = this.state;
    return (
